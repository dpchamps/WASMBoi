...
impl Mbc for Mbc1 {
    fn map_read(&self, address: u16) -> Result<u8, MbcError> {
        match address {
            ...
            0x4000..=0x7FFF => Ok(self.rom[(self.rom_bank_offset + address) as usize]),
            0xA000..=0xBFFF => {
    ...
    fn map_write(&mut self, address: u16, data: u8) -> Result<(), MbcError> {
        match address {
            ...
            0x2000..=0x3FFF => {
                ...
                self.rom_bank_offset = (self.rom_bank as u16) * 0x4000;
            ...
            0x4000..=0x5FFF => {
                if self.bank_mode {
                    ...
                    // Simple Banking Mode
                    self.rom_bank = (data & 0x60) + (self.rom_bank & 0x1f);
                    self.rom_bank_offset = (self.rom_bank as u16) * 0x4000;
                }
